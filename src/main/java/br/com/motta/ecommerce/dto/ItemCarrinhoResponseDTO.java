...
public record ItemCarrinhoResponseDTO (String nome, String apelido, String descricao, List<String> imagemUrl, Double preco, Double desconto, Integer quantidade) {

    public ItemCarrinhoResponseDTO(ItemCarrinho item){

        this(item.getProduto().getNome(),
                item.getProduto().getApelido(),
                item.getProduto().getDescricao(), item.getProduto().getImagemUrl(),
